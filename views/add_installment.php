...
// معالجة طلبات إضافة عميل جديد من الكاشير
if (isset($_GET['add_customer']) && $_GET['add_customer'] == '1') {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        try {
            // إضافة العميل الجديد
            $add_customer_sql = "INSERT INTO customers (name, phone, email, address) VALUES (?, ?, ?, ?)";
            $add_customer_stmt = $conn->prepare($add_customer_sql);
            $add_customer_stmt->execute([$customer_name, $customer_phone, $customer_email, $customer_address]);
            ...
            echo json_encode(['success' => false, 'error' => 'حدث خطأ أثناء إضافة العميل: ' . $e->getMessage()]);
            exit();
        }
    } else {
...
// التعامل مع الطلبات من الكاشير لإضافة مبلغ لحساب عميل
if (isset($_POST['from_cashier']) && $_POST['from_cashier'] == '1') {
    header('Content-Type: application/json');
    ...
    if (empty($customer_id)) {
        echo json_encode(['success' => false, 'error' => 'معرف العميل مطلوب']);
        exit();
    }
    
    if (empty($amount) || !is_numeric($amount) || $amount <= 0) {
        echo json_encode(['success' => false, 'error' => 'المبلغ غير صحيح']);
        exit();
    }
    
    if (empty($due_date)) {
        echo json_encode(['success' => false, 'error' => 'تاريخ الاستحقاق مطلوب']);
        exit();
    }
    ...
    try {
        ...
        if (!$customer) {
            $conn->rollBack();
            echo json_encode(['success' => false, 'error' => 'العميل غير موجود']);
            exit();
        }
        
        // إذا كان المبلغ خارجي (مبلغ إضافي)، نقوم بإضافته إلى الحساب
        if ($is_external_amount) {
            $new_account_price = ($customer['account_price'] ?? 0) + $amount;
        ...
        echo json_encode([
            'success' => true, 
            'message' => 'تم إضافة المبلغ لحساب العميل بنجاح',
            'customer_id' => $customer_id,
            'new_account_price' => $new_account_price
        ]);
        exit();
        ...
        $conn->rollBack();
        echo json_encode(['success' => false, 'error' => 'حدث خطأ أثناء إضافة المبلغ: ' . $e->getMessage()]);
        exit();
    }
}
